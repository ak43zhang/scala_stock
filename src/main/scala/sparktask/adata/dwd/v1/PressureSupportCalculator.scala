...
object PressureSupportCalculator {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      // 数据加载与校验
      val rawDS = loadAndValidateData(spark, inputPath).persist(StorageLevel.MEMORY_AND_DISK_SER)
      ...
      // 结果校验与持久化
      validateAndSaveResults(resultDS, outputPath)
  ...
  def optimizeWindowParameters(ds: Dataset[EnhancedData]): Dataset[WindowParam] = {
    ...
    // 参数候选范围（生产环境可配置化）
    val windowCandidates = (10 to 200 by 5).toArray
