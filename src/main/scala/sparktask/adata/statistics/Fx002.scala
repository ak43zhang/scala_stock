...
object Fx002 {
  def main(args: Array[String]): Unit = {
    ...
    val listsetDate = spark.sql("select trade_date from data_jyrl where  trade_date between '2025-03-13' and '2025-03-13' and trade_status='1' order by trade_date desc")
      .collect()
    ...
    for(setdate<-listsetDate){
      ...
      println(beforedate)
      // 筛选出前一天的风险数据
      val df4 = spark.sql(s"select stock_code,trade_time,channel_position,windowSize,'靠近压力位' as `风险类型` from ta3 where trade_time='$beforedate' and (support_ratio>=150 or support_ratio<0)")
...
//        spark.read.jdbc(url, table = s"($deleteQuery) AS subquery", properties)
      ...
      var df2:DataFrame = spark.read.jdbc(url, s"wencaiquery_venture_$setdate", properties).where("`风险类型` not like '%屠龙刀%'")
      ...
      resdf.select("`代码`").where("fxlx is null").repartition(1).write.mode("overwrite").option("encoding", "UTF-8")
        .text(s"file:///C:\\Users\\Administrator\\Desktop\\gs2024\\filter_table1\\$setdate\\过滤风险")

      resdf.where("fxlx is not null").repartition(1).write.mode("overwrite").option("encoding", "UTF-8")
        .csv(s"file:///C:\\Users\\Administrator\\Desktop\\gs2024\\filter_table1\\$setdate\\有风险")

      resdf.select("`代码`").where("fxlx is not null").repartition(1).write.mode("overwrite").option("encoding", "UTF-8")
        .text(s"file:///C:\\Users\\Administrator\\Desktop\\gs2024\\filter_table1\\$setdate\\有风险dm")
