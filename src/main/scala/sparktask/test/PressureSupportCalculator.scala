 ...
 */
object PressureSupportCalculator {
  // 在类顶部添加隐式排序定义
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      // 数据加载与校验
      val rawDS = loadAndValidateData(spark, "file:///D:\\gsdata\\gpsj_day_all_hs\\trade_date_month=202[4,5]*")
      ...
      // 结果校验与持久化
      validateAndSaveResults(resultDS, "file:///D:\\gsdata\\pressure_support_calculator")
  ...
  /** 数据加载与校验 */
  def loadAndValidateData(spark: SparkSession, path: String): Dataset[RawData] = {
    import spark.implicits._
    ...
    spark.read.parquet(path)
      .select(
        ...
        $"open".cast(DecimalType(18, 4)),
        $"high".cast(DecimalType(18, 4)),
        $"low".cast(DecimalType(18, 4)),
        $"close".cast(DecimalType(18, 4)),
  ...
  def optimizeWindowParameters(ds: Dataset[EnhancedData]): Dataset[WindowParam] = {
    ...
    // 参数候选范围（生产环境可配置化）
    val windowCandidates = (10 to 60 by 5).toArray
  ...
  /** 结果校验与存储 */
  private def validateAndSaveResults(ds: Dataset[ResultData], path: String): Unit = {
    // 数据校验
    val badRecords = ds.filter(row =>
