  ...
  def main(args: Array[String]): Unit = {
    ...
    import org.apache.spark.sql.{DataFrame, SparkSession}
        ...
        val excelDF: DataFrame = spark.read
          ...
          .option("dataAddress", "'条件20241210_after'!A1:BG5000") // 'page1'Sheet页名称(也可以选用'sheetName'的方式进行配置),A3代表从第几行读取(3则代表从第三行),E6代表行的范围
          .option("treatEmptyValuesAsNulls", "true") // 空值是否为作为null
          .option("inferSchema", "true")
          .load("file:///C:\\Users\\Administrator\\Desktop\\gs2024\\条件选股\\条件20241210_after.xlsx") // 如果是本地文件需要标注'file:///实际路径'因为spark会默认将HDFS作为文件系统
