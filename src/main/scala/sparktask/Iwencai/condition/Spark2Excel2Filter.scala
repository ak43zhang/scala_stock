...
object Spark2Excel2Filter {
  def main(args: Array[String]): Unit = {
    ...
    val fileName = "条件20241212_first"
    val dmName = "dm20241212"

    val excelDF: DataFrame = spark.read
      .format("com.crealytics.spark.excel")
      //      .option("sheetName", "Sheet1") // 读取的Sheet页
      .option("header", "true") // 第一行不作为表头,如果为true则作为表头
      .option("dataAddress", s"$fileName!A1:BG5000") // 'page1'Sheet页名称(也可以选用'sheetName'的方式进行配置),A3代表从第几行读取(3则代表从第三行),E6代表行的范围
...
//        from ta1
//        ) where gy=1 and dy=1 and yl/zc>1 and jcsl>=2
//        order by jcsl desc ,yl/zc desc
//        """.stripMargin)

    //收盘价,目前差比定在1，根据20241209数据可知，越大，胜率越高
        val df = spark.sql(
          """
            select *,yl/zc as cb from (
