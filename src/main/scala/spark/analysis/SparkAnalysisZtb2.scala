 ...
 *        涨跌幅差
 *        压力支撑位置(40日)
 *        压力支撑位120日（使用周期最大最小值作为压力支撑位）
 * 2、风险关联
 *
 * 3、热度关联
 *
 */
object SparkAnalysisZtb2 {
...
//    ps_df.printSchema()
    ...
    for (jyrl <- jyrls) {
      ...
      val n_day_ago = date_list.toList(40)
      println("yes_day:"+yes_day+",yes_day_5:"+yes_day_5+",n_day_ago:"+n_day_ago)
...
//      println("venture_year_df-----------"+venture_year_df.count())
      ...
      val risk_df = spark.sql(
        """
          ...
          |""".stripMargin)
      risk_df.where("`风险类型` not in ('预约披露时间_巨潮资讯')")//'股东大会'
      ...
      val ztb_xj_df = spark.sql(
        s"""
          ...
          group by `股票代码`,`股票简称`,max_date
           """.stripMargin).where("zdf is not null").orderBy("zdf")
      println("========================================================================================ztb_xj_df")
...
//      println(ztb_xj_df.count())
      ...
      val ztb_shuffer_df = spark.sql(
        """
          |select * from ztb_xj left join ps40 on ztb_xj.`股票代码` = ps40.stock_code
          |""".stripMargin)
...
//      println(ztb_shuffer_df.count())
      ...
      //基础数据对涨停板以及压力支撑位进行过滤
      val base_filter_ps40_df = spark.sql(
        """
          |select * from basequery as b left join ztb_shuffer as z on b.`代码`=z.`股票代码`
          |where `股票代码` is not null
          |""".stripMargin)
      println("========================================================================================base_filter_ps40_df")
//      base_filter_ps40_df.show(1000,false)
//      println(base_filter_ps40_df.count())
      base_filter_ps40_df.createOrReplaceTempView("bqps")
...
//      notice_lh_df.show()
      ...
      val result_df = spark.sql(
        s"""
          ...
          |if(fxdx_lh is not null,1,0) as fxdx_lh,
          ...
          |order by t1_spzf desc
          |""".stripMargin)
        .where(s"trade_time is not null") //${rd_count}
        .drop("股票代码","股票简称")
