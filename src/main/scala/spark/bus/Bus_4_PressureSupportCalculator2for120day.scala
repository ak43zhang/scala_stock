 ...
 */
object Bus_4_PressureSupportCalculator2for120day {
  // 在类顶部添加隐式排序定义
  ...
  def psc120(spark:SparkSession,properties:Properties,start_time:String,end_time:String): Unit ={
  ...
  def optimizeWindowParameters(ds: Dataset[EnhancedData]): Dataset[WindowParam] = {
    ...
    // 参数候选范围（生产环境可配置化）
    val windowCandidates = (120 to 120 by 5).toArray
