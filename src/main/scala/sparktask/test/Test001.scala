...
object Test001 {
  def main(args: Array[String]): Unit = {
    ...
    var risk1_df: DataFrame = spark.read.jdbc(url, s"$r1_table", properties)
    risk1_df.createOrReplaceTempView("risk1")

    var risk2_df: DataFrame = spark.read.jdbc(url, s"$r2_table", properties)
    risk2_df.createOrReplaceTempView("risk2")

//    var jhsaggg_df: DataFrame = spark.read.jdbc(url, s"$jhsaggg_table", properties)
//      .where("`代码` rlike '^(00|60)' and `名称` not like '%st%' and `名称` not like '%ST%' and `公告标题` not like '%st%' and `公告标题` not like '%ST%'")
//    jhsaggg_df.createOrReplaceTempView("jhsaggg")

    var jhsaggg_df: DataFrame = spark.read.jdbc(url, s"jhsaggg2025", properties)
      .union(spark.read.jdbc(url, s"jhsaggg2015", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2016", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2017", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2018", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2019", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2020", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2021", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2022", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2023", properties))
      .union(spark.read.jdbc(url, s"jhsaggg2024", properties))
      .where("`代码` rlike '^(00|60)' and `名称` not like '%st%' and `名称` not like '%ST%' and `公告标题` not like '%st%' and `公告标题` not like '%ST%'")
    jhsaggg_df.createOrReplaceTempView("jhsaggg")
...
//      s"""
//         select `代码`,`公告日期`,`公告标题`,'公告风险' as `风险类型`,extract_all_matches(`公告标题`, '$pattern') AS B from jhsaggg
...
//    val gl_df = spark.sql(
//      """
//        |select * from ztb_day2 as z left join hit as h on z.`股票代码`=h.`代码` and z.trade_date>h.`公告日期` and z.yes_day<=h.`公告日期`
//        |where trade_date between '2015-01-01' and '2025-04-25' and `代码` is not null
//        |order by trade_date desc,`股票代码` desc
//        |""".stripMargin)
//    gl_df.show(100,false)
...
//    spark.sql(
//      """
//        |select h,count(1) from gl lateral view explode(B) as h group by h order by count(1) desc
//        |""".stripMargin).show(1000,false)
//
//    spark.sql(
//      """
//        |select count(1) from gl lateral view explode(B) as h
//        |""".stripMargin).show(1000,false)
//
//    println(spark.sql(
//      """
//        |select trade_date,`股票代码`,count(1) from gl lateral view explode(B) as h group by trade_date,`股票代码`
//        |""".stripMargin).count())
